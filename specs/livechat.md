...
- **Description:** The LiveChat page allows Company Owner, Admin, and Agent users to manage, track, and reply to customer conversations from connected LiveChat widgets/channels.
...
| **LiveChat URL** | `https://dev.prowhats.com/en/contacts/livechat` |
...
5. Navigate manually or via menu to the LiveChat URL: `https://dev.prowhats.com/en/contacts/livechat`.

### 3.2 Default Page Loading Behavior
When the LiveChat page initializes, the system enforces the following default state:
...
| :--- | :--- | :--- | :--- |
| **All Messages** | **All** | Displays all customer conversations where messages have been sent or received via the LiveChat channel. | Owner, Admin, Agent |
...
- **Action Sequence:** 
  1. User clicks the **"Assigned to me"** button.
...
- **Expected System Behavior:** 
  - The message input viewport must be set to a disabled state, OR
...
### 6.2 Restriction: 24-Hour Policy Window
...
- **Messaging Barriers:** Outbound communication attempts must fail validation if the chat is unassigned or if the 24-hour session window has elapsed.
